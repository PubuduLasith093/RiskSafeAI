...
os.environ.setdefault("PYTHONPATH", str(pathlib.Path(__file__).resolve().parents[1] / "compliance_chat"))
...
def stub_model_loader(monkeypatch):
    # Patch both module paths to cover imports via `utils.model_loader` and `compliance_chat.utils.model_loader`
    import utils.model_loader as ml_mod
    from compliance_chat.utils import model_loader as ml_mod2

    class FakeApiKeyMgr:
        def __init__(self):
            self.api_keys = {"GROQ_API_KEY": "x", "GOOGLE_API_KEY": "y", "PINECONE_API_KEY": "z"}
    ...
    class FakeModelLoader:
        def __init__(self):
            ...
            self.config = {
                ...
                "pinecone": {"index_name": "test-index", "environment": "test"},
                "llm": {
                    "google": {
                        "provider": "google",
                        "model_name": "fake-llm",
            ...
            self.pc = None  # Stub Pinecone client
    ...
    import compliance_chat.src.document_ingestion.data_ingestion as di
    import compliance_chat.src.document_chat.retrieval as r
...
def stub_rag(monkeypatch):
    ...
    class FakeRAG:
        ...
        def load_retriever_from_pinecone(self, namespace, **kwargs):
            return None
